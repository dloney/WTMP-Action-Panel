...
import usbr.wat.plugins.actionpanel.model.planning.BcData;              // Provides BcData, the boundary condition data model used to detect met data dependencies
...
public class MeteorologyPanel extends AbstractPlanningPanel<MeteorlogicData> {
	 ...
	 */
	private static final String CONFIG_FILE = PlanningConfigFiles.getRelativeMetEditorFile();
	...
	/**
	 * Constructs a new MeteorologyPanel and wires it to the parent PlanningPanel.
	 *
	 * @param planningPanel the parent PlanningPanel that owns and displays this tab
	 */
	public MeteorologyPanel(PlanningPanel planningPanel) {
		super(planningPanel);
	}
	...
	protected void buildLowerPanel(EnabledJPanel lowerPanel) {
		String[] headers = new String[]{"Met Planning Name", "Type", "Description", "Planning Date"};
	 ...
	 * no PlanningSimulationGroup is currently loaded.
	 */
	@Override
	...
	/**
	 * Removes the given MeteorlogicData entry from the active PlanningSimulationGroup's
	 ...
	 */
	@Override
	protected void removeData(PlanningSimulationGroup fsg, MeteorlogicData data) {
		fsg.removeMetData(data);
	}

	/**
	 * Opens the ImportMetDataWindow to let the user select and import one or more
	 * meteorologic datasets into the current PlanningSimulationGroup. If a pre-constructed
	 ...
	 *
	 * @return the PlanningTable (_metTable) used as the meteorology data table
	 */
	@Override
	...
	/**
	 * Saves the current meteorology table contents back to the planning simulation group.
	...
	protected void savePanel() {
		// Only proceed if a valid planning simulation group is available
		if (_fsg != null) {
	...
	/**
	 * Loads the panel for the given PlanningSimulationGroup: populates the upper met table
	 ...
	 *
	 * @param fsg the PlanningSimulationGroup whose met data should be displayed, or null to disable
	 */
	@Override
	...
	protected void tableRowSelected(int selRow) {
		if (_fsg != null) {
			...
			if (selRow > -1) {
				...
				// Configure the plot for the selected dataset's planning year
	...
	private Object getMetConfigFileName(String metConfigFile) {
		...
		if (metConfigFile == null) {
			metConfigFile = PlanningConfigFiles.getRelativeHistoricalMetFile();
		}
